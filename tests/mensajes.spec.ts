...
test('login, search, add to cart, go to home and click item with fa fa-map-marker', async ({ page }) => {
    ...
    // Esperar a que el enlace de WhatsApp con el href 'https://api.whatsapp.com/send?&phone=523317843484' esté visible
    const whatsappLinkSelector = 'a[href="https://api.whatsapp.com/send?&phone=523317843484"]';
    await page.waitForSelector(whatsappLinkSelector, { state: 'visible' });

    // Hacer clic en el enlace de WhatsApp
    await page.click(whatsappLinkSelector);
